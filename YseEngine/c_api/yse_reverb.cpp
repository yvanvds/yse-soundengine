...
YSE_C_API void  yse_reverb_set_size(YseReverb* rev, float v)        { if (rev) to_cpp(rev)->setSize(v); }
YSE_C_API float yse_reverb_get_size(YseReverb* rev)                 { return rev ? to_cpp(rev)->getSize() : 0.0f; }
YSE_C_API void  yse_reverb_set_roll_off(YseReverb* rev, float v)    { if (rev) to_cpp(rev)->setRollOff(v); }
YSE_C_API float yse_reverb_get_roll_off(YseReverb* rev)             { return rev ? to_cpp(rev)->getRollOff() : 0.0f; }
YSE_C_API void  yse_reverb_set_active(YseReverb* rev, int on)       { if (rev) to_cpp(rev)->setActive(on != 0); }
YSE_C_API int   yse_reverb_get_active(YseReverb* rev)               { return rev && to_cpp(rev)->getActive() ? 1 : 0; }
YSE_C_API void  yse_reverb_set_room_size(YseReverb* rev, float v)   { if (rev) to_cpp(rev)->setRoomSize(v); }
YSE_C_API float yse_reverb_get_room_size(YseReverb* rev)            { return rev ? to_cpp(rev)->getRoomSize() : 0.0f; }
YSE_C_API void  yse_reverb_set_damping(YseReverb* rev, float v)     { if (rev) to_cpp(rev)->setDamping(v); }
YSE_C_API float yse_reverb_get_damping(YseReverb* rev)              { return rev ? to_cpp(rev)->getDamping() : 0.0f; }
...
YSE_C_API float yse_reverb_get_dry(YseReverb* rev) { return rev ? to_cpp(rev)->getDry() : 0.0f; }
YSE_C_API float yse_reverb_get_wet(YseReverb* rev) { return rev ? to_cpp(rev)->getWet() : 0.0f; }
