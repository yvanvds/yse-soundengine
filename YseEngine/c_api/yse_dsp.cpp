...
YSE_C_API YseDspBuffer* yse_dsp_buffer_create(unsigned int length, unsigned int overflow) {
  try { return reinterpret_cast<YseDspBuffer*>(new YSE::DSP::buffer(length, overflow)); }
  catch (const std::exception& e) { yse_c::set_last_error(e.what()); return nullptr; }
  catch (...) { yse_c::set_last_error("buffer_create: unknown C++ exception"); return nullptr; }
}

YSE_C_API YseDspBuffer* yse_dsp_drawable_buffer_create(unsigned int length, unsigned int overflow) {
  try { return reinterpret_cast<YseDspBuffer*>(new YSE::DSP::drawableBuffer(length, overflow)); }
  catch (const std::exception& e) { yse_c::set_last_error(e.what()); return nullptr; }
  catch (...) { yse_c::set_last_error("drawable_buffer_create: unknown C++ exception"); return nullptr; }
}

YSE_C_API YseDspBuffer* yse_dsp_file_buffer_create(unsigned int length, unsigned int overflow) {
  try { return reinterpret_cast<YseDspBuffer*>(new YSE::DSP::fileBuffer(length, overflow)); }
  catch (const std::exception& e) { yse_c::set_last_error(e.what()); return nullptr; }
  catch (...) { yse_c::set_last_error("file_buffer_create: unknown C++ exception"); return nullptr; }
}

YSE_C_API YseDspBuffer* yse_dsp_wavetable_create(unsigned int length) {
  try { return reinterpret_cast<YseDspBuffer*>(new YSE::DSP::wavetable(length)); }
  catch (const std::exception& e) { yse_c::set_last_error(e.what()); return nullptr; }
  catch (...) { yse_c::set_last_error("wavetable_create: unknown C++ exception"); return nullptr; }
}
...
YSE_C_API unsigned int yse_dsp_buffer_length(YseDspBuffer* buf)        { return buf ? to_cpp(buf)->getLength()    : 0; }
YSE_C_API unsigned int yse_dsp_buffer_length_ms(YseDspBuffer* buf)     { return buf ? to_cpp(buf)->getLengthMS()  : 0; }
YSE_C_API float        yse_dsp_buffer_length_sec(YseDspBuffer* buf)    { return buf ? to_cpp(buf)->getLengthSec() : 0.0f; }
YSE_C_API int          yse_dsp_buffer_is_silent(YseDspBuffer* buf)     { return buf && to_cpp(buf)->isSilent() ? 1 : 0; }
YSE_C_API float        yse_dsp_buffer_max_value(YseDspBuffer* buf)     { return buf ? to_cpp(buf)->maxValue() : 0.0f; }
YSE_C_API float        yse_dsp_buffer_get_back(YseDspBuffer* buf)      { return buf ? to_cpp(buf)->getBack()  : 0.0f; }
...
YSE_C_API YseStatus yse_dsp_buffer_draw_line(YseDspBuffer* buf, unsigned int start, unsigned int stop, float start_value, float stop_value) {
  ...
  if (!d) { yse_c::set_last_error("buffer is not a drawableBuffer"); return YSE_ERR_INVALID_HANDLE; }
...
YSE_C_API YseStatus yse_dsp_buffer_load_file(YseDspBuffer* buf, const char* filename, unsigned int channel) {
  ...
  if (!f) { yse_c::set_last_error("buffer is not a fileBuffer"); return YSE_ERR_INVALID_HANDLE; }
...
YSE_C_API YseStatus yse_dsp_wavetable_create_saw(YseDspBuffer* buf, int harmonics, int length) {
  ...
  if (!w) { yse_c::set_last_error("buffer is not a wavetable"); return YSE_ERR_INVALID_HANDLE; }
