...
void YSE::channel::registerOnBus()
{
	...
	if (!claimChannelName(busName))
	{
		INTERNAL::LogImpl().emit(E_FILE_ERROR,
			"channel name '" + busName + "' is already in use; bus registration rejected");
		return;
	}
	busOwner = true;
	...
	busVolumeHandle = INTERNAL::Bus().subscribe("channel." + busName + ".volume",
		[this](const BusValue& v) {
			if (auto* f = std::get_if<float>(&v)) { if (isValid()) setVolume(*f); }
			else if (auto* i = std::get_if<int>(&v)) { if (isValid()) setVolume(static_cast<float>(*i)); }
		});
}
