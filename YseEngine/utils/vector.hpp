...
namespace YSE {
  ...
  public:
    ...
    float length() {
      return (float)sqrt(std::pow(x, 2) + std::pow(y, 2) + std::pow(z, 2));
    }
    ...
    friend Pos operator+(const Pos& v, float r) {
      return Pos(v.x + r, v.y + r, v.z + r);
    }
    friend Pos operator-(const Pos& v, float r) {
      return Pos(v.x - r, v.y - r, v.z - r);
    }
    ...
    friend Pos operator+(float r, const Pos& v) {
      return Pos(r + v.x, r + v.y, r + v.z);
    }
    friend Pos operator-(float r, const Pos& v) {
      return Pos(r - v.x, r - v.y, r - v.z);
    }
  ...
  inline Int Min(Int x, UInt y) {
    return (x < static_cast<Int>(y)) ? x : static_cast<Int>(y);
  }
  inline UInt Min(UInt x, UInt y) {
    return (x < y) ? x : y;
  }
  inline I64 Min(Int x, I64 y) {
    return (static_cast<I64>(x) < y) ? static_cast<I64>(x) : y;
  }
  inline Flt Min(Flt x, Flt y) {
    return (x < y) ? x : y;
  }
  inline Flt Min(Int x, Flt y) {
    return (static_cast<Flt>(x) < y) ? static_cast<Flt>(x) : y;
  }
  inline Flt Min(Flt x, Int y) {
    return (x < static_cast<Flt>(y)) ? x : static_cast<Flt>(y);
  }
  ...
  inline UInt Max(UInt x, Int y) {
    return (x > static_cast<UInt>(y)) ? x : static_cast<UInt>(y);
  }
  ...
  inline float Dist(const Pos& a, const Pos& b) {
    return sqrt(static_cast<float>(std::pow((b.x - a.x), 2) + std::pow((b.y - a.y), 2) +
                                   std::pow((b.z - a.z), 2)));
  }
