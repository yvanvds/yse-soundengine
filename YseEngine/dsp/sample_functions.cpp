...
file.getFullPathName().toStdString()); return nullptr;
        }
...
bool YSE::DSP::LoadFromFile(const char* /*fileName*/, YSE::DSP::buffer& /*buffer*/,
                            UInt /*channel*/) {
  ...
  buffer.setSampleRateAdjustment(static_cast<Flt>(reader->sampleRate) /
...
bool YSE::DSP::LoadFromFile(const char* /*fileName*/, MULTICHANNELBUFFER& /*buffer*/) {
  ...
  for (unsigned int i = 0; i < buffer.size(); i++) {
    buffer[i].setSampleRateAdjustment(static_cast<Flt>(reader->sampleRate) /
  static_cast<Flt>(SAMPLERATE)); buffer[i].resize(tBuf.getNumSamples());
...
bool YSE::DSP::SaveToFile(const char* /*fileName*/, MULTICHANNELBUFFER& /*buffer*/) {
  ...
  if (IO().getActive()) {
    ...
    if (fileStream != nullptr) {
      WavAudioFormat wavFormat;
      AudioFormatWriter * writer = wavFormat.createWriterFor(fileStream, SAMPLERATE, buffer.size(),
