...
CONSTRUCT() {
  ...
  ADD_DESCRIPTION(
      ...
      "construction. .preset itself is not settable, so no preset captures a preset; the "
      "pre-protocol scalar controls do not hold the round trip and are not captured (opting one "
      "in is that object's own migration, issue #846); objects nested in subpatchers participate "
