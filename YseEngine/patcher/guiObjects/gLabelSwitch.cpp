...
gLed::gLed() : gLabelSwitchBase() {
  Document(
      ...
      "is '0' or '1' rather than .t's 'on' / 'off', which is exactly what lets this object promise "
      "GuiValueIsSettable() where .b and .t cannot - inlet 0 takes a number, so the string a host "
      "polls is the string it can push back, and .preset can store and restore a named switch. In "
