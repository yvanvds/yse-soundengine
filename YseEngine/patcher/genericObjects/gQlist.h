...
namespace YSE {
  namespace PATCHER {
     ...
     *
     *  ``.textfile``'s answer, for ``.textfile``'s reason, and it is not a
     *  matter of effort. File I/O cannot be done from a patcher message handler
     *  at all today: a handler runs on whichever thread the message arrived on,
     ...
     *  callback. Opening a file there would block it.
     *
     *  Doing it properly needs a background job, an object lifetime that
     *  outlives the job, the host's ``IO()`` virtual-file layer and a completion
     *  delivered back into a real dispatch frame. That is shared plumbing —
     *  ``.coll`` and ``.textfile`` need the identical thing — and it is issue
     *  **#683**, which builds it; **#689** is this object's half. Max's third
     *  outlet, which bangs when a file has been read, has nothing to fire until
     *  then and is therefore left off; when it lands it is **appended** as
     *  outlet 2, which is also Max's position for it, so no saved patch's cords
     *  shift either way.
     *
     *  ### Reserved words, and the semicolon
     *
     *  ``bang``, ``next``, ``fwd``, ``rewind``, ``stop``, ``clear``, ``set``,
     *  ``append``, ``insert`` and ``tempo`` are commands; ``read``, ``write``,
