...
void DemoVirtual::AddSound()
{
	for (int i = 0; i < 10; i++) {
		...
		switch (YSE::Random(4)) {
		case 0: sounds.front().create(YSE_TEST_RESOURCES_DIR "/contact.ogg", &YSE::ChannelAmbient(), true); break;
		case 1: sounds.front().create(YSE_TEST_RESOURCES_DIR "/drone.ogg", &YSE::ChannelVoice(), true); break;
		case 2: sounds.front().create(YSE_TEST_RESOURCES_DIR "/kick.ogg", &YSE::ChannelMusic(), true); break;
		case 3: sounds.front().create(YSE_TEST_RESOURCES_DIR "/pulse1.ogg", &YSE::ChannelFX(), true); break;
		}
