...
TEST_CASE("log class: log of in1 with base in2") {
    ...
    in1.getPtr()[0] = 8.0f;     in2.getPtr()[0] = 2.0f;   // log2(8) = 3
    in1.getPtr()[1] = 100.0f;   in2.getPtr()[1] = 10.0f;  // log10(100) = 2
    YSE::DSP::log l;
...
TEST_CASE("inverter: default (zeroToOne=false) negates each sample (large buffer covers unrolled loop)") {
    // > 8 samples so both the unrolled 8-at-a-time loop and the tail run.
    constexpr unsigned N = 11;
    YSE::DSP::buffer in(N);
    for (unsigned i = 0; i < N; ++i) in.getPtr()[i] = static_cast<float>(i + 1);
...
TEST_CASE("inverter: zeroToOne=true computes 1 - sample (large buffer covers unrolled loop)") {
    constexpr unsigned N = 11;
    YSE::DSP::buffer in(N);
    for (unsigned i = 0; i < N; ++i) in.getPtr()[i] = 0.1f * static_cast<float>(i);
    YSE::DSP::inverter inv;
