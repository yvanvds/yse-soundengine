...
TEST_CASE("fft: Parseval's theorem — freq energy equals N times time energy") {
    ...
    for (unsigned i = 0; i < N; ++i) timeEnergy += rp[i] * rp[i];
    ...
    for (unsigned i = 0; i < N; ++i) freqEnergy += fr[i] * fr[i] + fi[i] * fi[i];
...
TEST_CASE("realFft: peak bin matches input sine frequency") {
    ...
    unsigned peak = TestHelpers::peakBinIndex(
        rf.getReal().getPtr(), rf.getImaginary().getPtr(), N);
    CHECK(peak == k);
}
...
TEST_CASE("mayer_realfft / mayer_realifft: round-trip scales by N") {
    ...
    for (int i = 0; i < N; ++i)
        orig[i] = data[i] = std::sin(2.0f * kPi * 5.0f * static_cast<float>(i) / static_cast<float>(N));
