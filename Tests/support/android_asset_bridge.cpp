...
namespace yse_tests {
  ...
  namespace {
    ...
    bool ensureDir(const std::string& path) {
      ...
      __android_log_print(ANDROID_LOG_ERROR, kLogTag, "mkdir(%s) failed: %s", path.c_str(),
                          std::strerror(errno));
      return false;
    }

    bool copyAssetToFile(AAssetManager* mgr, const std::string& assetPath,
                         const std::string& outPath) {
      AAsset* asset = AAssetManager_open(mgr, assetPath.c_str(), AASSET_MODE_STREAMING);
      if (!asset) {
        __android_log_print(ANDROID_LOG_ERROR, kLogTag, "AAssetManager_open(%s) returned null",
                            assetPath.c_str());
        return false;
      }
      ...
      if (!out) {
        __android_log_print(ANDROID_LOG_ERROR, kLogTag, "fopen(%s) failed: %s", outPath.c_str(),
                            std::strerror(errno));
        AAsset_close(asset);
        return false;
      }
      ...
      while ((n = AAsset_read(asset, buf, sizeof(buf))) > 0) {
        if (std::fwrite(buf, 1, (size_t)n, out) != (size_t)n) {
          __android_log_print(ANDROID_LOG_ERROR, kLogTag, "fwrite(%s) short write",
                              outPath.c_str());
          ok = false;
          break;
        }
    ...
    bool extractDirRecursive(AAssetManager* mgr, const std::string& assetDir,
  ...
  bool extractFixturesFromAssets(struct android_app* app) {
    ...
    __android_log_print(ANDROID_LOG_INFO, kLogTag, "extracting fixtures from APK assets to %s",
