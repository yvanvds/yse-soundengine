...
YSE::INTERNAL::NamedBus& bus() { return YSE::INTERNAL::Bus(); }
...
struct ScopedSub {
  ...
  ~ScopedSub() { bus().unsubscribe(handle); }
...
bool pumpUntil(Pred pred, int tries = 300, unsigned ms = 10) {
...
void run(const char* src) { yse_run_script(src); }
