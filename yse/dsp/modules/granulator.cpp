...
: poolSize(poolSize), maxGrains(maxGrains)
, leftOverStarts(0.f), parmGain(1.0f), parmTranspose(0), parmTransposeRandom(0) {
...
void YSE::DSP::MODULES::granulator::process(MULTICHANNELBUFFER & buffer) {
  ...
  if(num) for (UInt i = 0; i < grainPool->size(); i++) {
    if (grainPool->at(i).state == GS_STOPPED) {
      ...
      if (parmLengthRandom) length += Random(-parmLengthRandom, parmLengthRandom);
