...
void YSE::DSP::ADSRenvelope::generate() {
  ...
  envelope.resize(breakPoints.back().time * SAMPLERATE);

  Flt * ptr = envelope.getPtr();
  UInt currentPoint = 0;
  UInt lastTarget = 0;
  UInt target = breakPoints[1].time * SAMPLERATE;
  Flt value;

  for (int i = 0; i < envelope.getLength(); i++) {
    *ptr++ = breakPoints[currentPoint].value
    ...
    if (i == target) {
      lastTarget = target;
      currentPoint++;
      target = breakPoints[currentPoint + 1].time * SAMPLERATE;
