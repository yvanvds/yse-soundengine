...
YSE::DSP::lfo::lfo() : cursor(0.f), previousType(LFO_NONE) {
  ...
  if (LfoSawTable.getLength() == 0) {
    ...
    LfoTriangleTable.drawLine(0, SAMPLERATE * 0.5f, 0.f, 1.f);
    LfoTriangleTable.drawLine(SAMPLERATE * 0.5f, SAMPLERATE, 1.f, 0.f);

    LfoSineTable.resize(SAMPLERATE);
    // perhaps this is a bit clumsy...
    sine s;
    for (int i = 0; i < SAMPLERATE; i += STANDARD_BUFFERSIZE) {
